...
PRECISION = 'precision_at_1-csls_knn_10'
DIS_ACC = 'dis_accu'
# main
...
parser.add_argument("--export", type=bool_flag, default=False, help="Export embeddings after training")
...
# map_beta 0.01-0.001 0.0001 0.00001
parser.add_argument("--map_beta", type=float, default=0.0001, help="Beta for orthogonalization") #0.0001 for zh
# discriminator
parser.add_argument("--dis_layers", type=int, default=2, help="Discriminator layers")
#parser.add_argument("--dis_hid_dim", type=int, default=2048, help="Discriminator hidden layer dimensions")
parser.add_argument("--dis_hid_dim", type=int, default=500, help="Discriminator hidden layer dimensions")
parser.add_argument("--dis_dropout", type=float, default=0., help="Discriminator dropout")
parser.add_argument("--dis_input_dropout", type=float, default=0.1, help="Discriminator input dropout")
parser.add_argument("--dis_steps", type=int, default=5, help="Discriminator steps")
parser.add_argument("--dis_lambda", type=float, default=1, help="Discriminator loss feedback coefficient")
#parser.add_argument("--dis_most_frequent", type=int, default=75000, help="Select embeddings of the k most frequent words for discrimination (0 to disable)")
parser.add_argument("--dis_most_frequent", type=int, default=3000, help="Select embeddings of the k most frequent words for discrimination (0 to disable)")
...
parser.add_argument("--number", type=int, default=0, help="model number")
...
#params.exp_path = "dumped/debug/7umo2c8bsj/"
#params.exp_path = "dumped/debug/6y6sed951w/"
...
if params.adversarial:
    ...
    for n_epoch in range(params.n_epochs):
        ...
        cos_score.append(to_log[VALIDATION_METRIC])
        acc_score.append(to_log[PRECISION])
        dis_score.append(to_log[DIS_ACC])
...
# plot fig
fig = plt.figure()
plt.plot(range(0, len(cos_score)), cos_score, color='r', label='cos_score')
plt.ylabel('accuracy/loss')
plt.xlabel('epochs')
plt.legend()
fig.savefig('figures/cos_score{}.png'.format(params.number))


fig = plt.figure()
plt.plot(range(0, len(acc_score)), acc_score, color='r', label='acc_score')
plt.ylabel('accuracy/loss')
plt.xlabel('epochs')
plt.legend()
fig.savefig('figures/acc_score{}.png'.format(params.number))

fig = plt.figure()
plt.plot(range(0, len(dis_score)), dis_score, color='r', label='dis_acc')
plt.ylabel('accuracy/loss')
plt.xlabel('epochs')
plt.legend()
fig.savefig('figures/dis_score{}.png'.format(params.number))
